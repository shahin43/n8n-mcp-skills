...
**Expert AI coding agent skills for building flawless n8n workflows using the n8n-mcp MCP server**

For connecting with n8n instance, an mcp server can is used as 
[![n8n-mcp](https://img.shields.io/badge/n8n--mcp-compatible-green.svg)](#-what-is-this)


## 🎯 What is this?

This repository contains **8 complementary AI coding agent skills** that teach AI assistants (Claude Code, Codex, Cursor, and other MCP-compatible agents) how to build production-ready n8n workflows using the `n8n-mcp` MCP server.
...
These skills solve these problems by teaching AI agents:
...
Build production-ready `AI Agent` nodes with the correct prompt, model, tool, memory, parser, and fallback wiring.

**Activates when**: Creating AI Agent nodes, chat agents, action agents, memory-backed assistants, structured-output agents.

**Key Features**:
- Current AI Agent node constraints and wiring model
- System message and tool-description patterns
- Safe action-agent confirmation rules
- Output parser and fallback model guidance
- MCP connection patterns for AI Agent workflows
...
1. **n8n-mcp MCP server** installed and configured
2. An AI coding agent: **Claude Code**, **Codex CLI/App**, **Cursor**, or any MCP-compatible agent
3. `.mcp.json` configured with n8n-mcp server

### MCP Server Configuration

Copy `.mcp.json.example` to `.mcp.json` and fill in your n8n instance details:
...
```bash
# 1. Configure MCP server (copy and edit .mcp.json.example)
cp .mcp.json.example  .mcp.json
# Edit .mcp.json with your n8n instance URL and API key
# You can also run codex mcp --help or using ~/.codex/config.toml; for enabling codex mcp server

# 2. Codex will automatically read AGENTS.md for instructions
```
...
1. Clone the repository and configure `.mcp.json` as above
2. Cursor reads `AGENTS.md` automatically for project context
3. MCP tools become available through Cursor's MCP integration

### Claude.ai

1. Download individual skill folders from `n8n-skills/skills/`
2. Zip each skill folder
3. Upload via Settings → Capabilities → Skills

### API / SDK

See [n8n-skills/docs/INSTALLATION.md](n8n-skills/docs/INSTALLATION.md) for detailed instructions.
...
"Can I setup a workflow using AI agents in n8n"
→ Activates: n8n AI Agent Node Builder
```
...
- [Installation Guide](n8n-skills/docs/INSTALLATION.md) - Detailed installation for all platforms
- [Usage Guide](n8n-skills/docs/USAGE.md) - How to use skills effectively
- [Development Guide](n8n-skills/docs/DEVELOPMENT.md) - Contributing and testing
- [MCP Testing Log](n8n-skills/docs/MCP_TESTING_LOG.md) - Real tool responses used in skills
...
Contributions welcome! Please see [DEVELOPMENT.md](n8n-skills/docs/DEVELOPMENT.md) for guidelines.
...
- [![n8n Skills Intro](skills.png)](https://youtu.be/e6VvRqmUY2Y?si=6Igply3cadjO6Xx0)
